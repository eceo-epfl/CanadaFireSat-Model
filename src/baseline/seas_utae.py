...
class UTAEWrap(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *args):  # x here is B, T, H, W, C
        x = x.permute(0, 1, 4, 2, 3)
        B, T, C, H, W = x.shape
        batch_positions = torch.arange(T).unsqueeze(0).repeat(B, 1).to(x)
        x = self.net(x, batch_positions=batch_positions)
