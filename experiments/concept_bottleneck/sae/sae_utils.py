...
from overcomplete.metrics import r2_score, l1
...
def region_mse_bands_per_class(
    ...
    class_names: List[str] = ["background", "TC", "AR"]#["no_fire", "fire"]
) -> dict:
    """
...
def top_k_auxiliary_loss(x, x_hat, pre_codes, codes, dictionary, penalty=0.1, l1_penalty=0.,
                         scale=None, shift=None, mean=None, std=None):
    """
    ...
    if mean is not None and std is not None:
        residual_hat = residual_hat * (std + 1e-5) + mean
...
def ghost_grad_loss(x, x_hat, pre_codes, codes, dictionary, dead_neuron_mask, scale=None, shift=None,
                    penalty=0.1, ghost_penalty=1.0):
    ...
    if dead_neuron_mask is not None and dead_neuron_mask.sum() > 0:
        ...
        # print(f"Applying Ghost Gradient Loss on {dead_neuron_mask.sum().item()} dead neurons.")

        l2_norm_residual = torch.norm(residual, dim=-1)
        feature_acts_dead_neurons_only = torch.exp(
            pre_codes[:, dead_neuron_mask]
        )
        ghost_out = feature_acts_dead_neurons_only @ dictionary[dead_neuron_mask, :]

        # TODO: Prior of After the normalizations
        if scale is not None and shift is not None:
        ...
        # print(f"Ghost Gradient Loss Mean: {mse_loss_ghost_resid.item()}")
